...
洞察与用户研究 Agent：输入**关于人类行为与决策的问题**，Agent 自主完成用户研究全流程，为**驱动真实选择的主观因素**建模（动机/情感/权衡/认知偏差），输出深度洞察报告。
...
src/
  ...
  persona/     # 角色卡类型（zod schema）
  agent/       # 主循环：工具注册 + 阶段流转 + 发言器 + 讨论/访谈引擎
  source/      # 数据源抽象：360 搜索（联网侦察）+ 本地语料库（离线兜底）
test/          # 镜像 src/ 路径 + helpers（测试用假模型）
workspaces/    # 每次研究一个目录（已 gitignore）：<问题关键词>-<时间>，如 为什么学生不去图书馆-150230
logs/          # 运行日志（已 gitignore）：每个研究一份 JSONL，可用 jq/grep 检查
...
- v0.2.3 结果输出人性化：工作区目录名改为"问题关键词-时间"（可读可区分）；完成后打印问题/工作区/报告/素材路径与查看命令
