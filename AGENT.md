...
- **角色长期记忆**（v0.5.0 已落地）：每个角色独立记忆文件（notes/persona-memory/<id>.md），每轮后并行 LLM 摘要沉淀，发言前注入——角色记得自己说过的话（真实验证通过）。后续可加：记忆检索注入（而非全量）、记忆容量上限与归档。
...
- **讨论演进为真多 Agent 直接交互**（用户明确要求，重要方向）：现状=多角色模拟（单模型分饰多角，每次发言重建上下文）——信息流已交互（transcript 传递、互相回应）但状态不持续。目标=每个角色是独立 Agent（独立持久上下文/记忆），角色间直接对话交互，对齐 Atypica 企业级 Multi-Agent 编排。演进路径：① 角色长期记忆（v0.5.0 已落地）→ ② 每个角色独立会话状态管理（未做）→ ③ 角色间直接消息传递（A→B→C 实时回应链，而非引擎统一轮询，未做）。
- **数据源选择权交给 LLM**（用户 2026-08-09 提出）：当前 scout_sources 由代码写死遍历全部数据源（corpus+bilibili+360），LLM 无法抉择用什么源。目标：信息源选择成为 LLM 的决策——把每个源暴露为独立工具（scout_bilibili / scout_360 / scout_corpus），或让 scout_sources 接收 sources 参数，LLM 根据问题类型自主选择（问学生行为→B站评论，问行业趋势→360 报道），可多次调用组合多个源。
